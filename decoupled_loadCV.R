...
spec <- matrix(
	...
	'tempSource', 'ts', 2, 'character', # gam, lm, nnet, randomforest, mean, temp
	'tempModel', 'tm', 1, 'integer', # model of temp sources 1-4
...
temp.source <- 'ALL'
if(!is.null(opt$tempSource)) {
	temp.source <- opt$tempSource
}
...
#### HOW TO LOAD TEMP AND SPECIFY MODEL? ### 

# assign source temp models to 'temp.model.formulas' variable
if(temp.source== "gam") {
	temp.model.formulas <- gam.temp.models
} else if(pred.method == "lm") {
	temp.model.formulas <- lm.temp.models
} else if(pred.method == "nnet") {
	temp.model.formulas <- nnet.temp.models
} else if(pred.method == "randomforest") {
	temp.model.formulas <- randomforest.temp.models
}

temp.path <- paste(folder.path, "temp", sep="/")
dirs <- list.dirs(temp.path, full.names = TRUE, recursive=FALSE)
print(dirs)

if(temp.source != 'ALL') {
	temp.path <- paste(folder.path, temp.source, sep="/")
	temp.files <- dir(temp.method.path, pattern = '^predtrain(.*)temp-fit-target', full.names = TRUE)
	# search for pattern in list of strings
	print(temp.files)
	# get length of temp.source temp.models
	#readRDS()
}

index <- which(avg.temp$HASH==hashToDt(load.train.start.dt))
...
### CROSS VALIDATION ###
########################
num.temp.model <- length(temp.model.formulas)+2
temp.model.formulas[[num.temp.model-1]] <- "mean yearly lag temperature"
temp.model.formulas[[num.temp.model]] <- "true temperature"
for(i in 1:length(temp.model.formulas)+2) {
  PINBALL <- c()
  POSITIONS <- c()
