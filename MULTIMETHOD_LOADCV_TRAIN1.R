	...
	for(j in 1:length(temp.formulas)) {
		...
		for(p in 1:length(intervals)) {
			...
			if(grepl("(GAM|LM)", curr.temp.method.option)) {
				weekly.scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "csv")
				monthly.scores.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-scores", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "csv")
			} else if(no.temp.formula) {
				weekly.scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", load.method, "?", ptemp.method, sep="_"), "csv")
				monthly.scores.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-scores", load.method, "?", ptemp.method, sep="_"), "csv")
			} else {
				weekly.scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", load.method, "?", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "csv")
				monthly.scores.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-scores", load.method, "?", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "csv")
			}
			...
			print(date.period)
			if(grepl("(GAM|LM)", curr.temp.method.option)) {
				all.weekly.fits.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-series", load.method, date.period, "all", "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
				all.monthly.fits.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, date.period, "all", "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			} else if(no.temp.formula) {
				all.weekly.fits.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-series", load.method, date.period, "all", "?", "temp-model", ptemp.method, sep="_"), "rds")
				all.monthly.fits.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, date.period, "all", "?", "temp-model", ptemp.method, sep="_"), "rds")
			} else {
				all.weekly.fits.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-series", load.method, date.period, "all", "?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
				all.monthly.fits.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, date.period, "all","?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			}
			...
			for (ml in 1:test.month.len) {
				...
				for (h in 1:length(loop.vars)) { 
					...
					if(h==1) load.train.features <- assembleLoadFeatures(load.features, temp, load.train.dt, lag, load.train.month.len, 2)
					...
					if(h==1) {
						if(grepl("(GAM|LM)", curr.temp.method.option)) {
							load.model.fn <- extensionJoin(paste("monthly", "model", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], pred.type, "instance", ml, sep="_"), "txt")
						} else if(no.temp.formula) {
						...
						if(grepl("(GAM|LM)", curr.temp.method.option)) {
							load.model.fn <- extensionJoin(paste("weekly", "model", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], pred.type, "instance", ml, sep="_"), "txt")
						} else if(no.temp.formula) {
                    ...
                    load.train.features <- cbind(load.train.features, RES=train.residuals)
					...
					if(grepl("(GAM|LM)", curr.temp.method.option)) {
						monthly.res.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, as.Date(test.dt), "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], "instance", ml, sep="_"), "rds")
					} else if(no.temp.formula) {
						monthly.res.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, as.Date(test.dt), "?", "temp-model", ptemp.method, "instance", ml, sep="_"), "rds")
					} else {
						monthly.res.fn <- extensionJoin(paste("mtrain", "monthly", "load-fit-series", load.method, as.Date(test.dt),"?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], "instance", ml, sep="_"), "rds")
					}
			...
			position.board <- cbind(MAPE=res[[1]]$MAPE, PINBALL=res[[1]]$PINBALL, POSITION=POSITIONS, PINBALL_=firstpos_benchmark[1:test.month.len, ])
			dates <- res[[1]][1:(nrow(res[[1]])), c(1,2)]
            dates <- cbind(dates, MONTH=format(as.POSIXct(dates[1:(nrow(res[[1]])), 2], tz="EST"), "%b %y"))
			...
			for(h in 1:(length(loop.vars)+1)) {
				if (h==1) {
					...
					if(grepl("(GAM|LM)", curr.temp.method.option)) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "1m", load.method, date.period, "all", "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					} else if(no.temp.formula) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "1m", load.method, date.period, "all", "?", "temp-model", ptemp.method, sep="_"), "rds")
					} else {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "1m", load.method, date.period, "all", "?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					}					
					col <- position.board[1:test.month.len, 1:7]
					comp <- col
				} else if (h==2) {
				    appendTableToFile(score.board[[h]], weekly.scores.path)
				    cat("\n", file = weekly.scores.path, append = TRUE)
					if(grepl("(GAM|LM)", curr.temp.method.option)) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "5wrest", load.method, date.period, "all", "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					} else if(no.temp.formula) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "5wrest", load.method, date.period, "all", "?", "temp-model", ptemp.method, sep="_"), "rds")
					} else {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", "5wrest", load.method, date.period, "all", "?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					}	
				} else {
				    appendTableToFile(score.board[[h]], weekly.scores.path)
				    cat("\n", file = weekly.scores.path, append = TRUE)
					...
					if(grepl("(GAM|LM)", curr.temp.method.option)) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", getPredictionType(htype, test.horizon), load.method, date.period, "all", "?", "temp-model", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					} else if(no.temp.formula) {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", getPredictionType(htype, test.horizon), load.method, date.period, "all", "?", "temp-model", ptemp.method, sep="_"), "rds")
					} else {
						scores.fn <- extensionJoin(paste("mtrain", "weekly", "load-fit-scores", getPredictionType(htype, test.horizon), load.method, date.period, "all", "?", "temp-model", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
					}	
					sb <- score.board[[h]]
			...
			if(grepl("(GAM|LM)", curr.temp.method.option)) {
				pos.board.fn <- extensionJoin(paste("mtrain", "position_board", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			} else if(no.temp.formula) {
				pos.board.fn <- extensionJoin(paste("mtrain", "position_board", load.method, "?", ptemp.method, sep="_"), "rds")
			} else {
				pos.board.fn <- extensionJoin(paste("mtrain", "position_board", load.method, "?", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			}
			saveRDS(position.board, file=pathJoin(load.temp.scores.path, pos.board.fn), compress=TRUE)
			...
			if(grepl("(GAM|LM)", curr.temp.method.option)) {
				comp.board.fn <- extensionJoin(paste("mtrain", "comparison_board", load.method, "?", ptemp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			} else if(no.temp.formula) {
				comp.board.fn <- extensionJoin(paste("mtrain", "comparison_board", load.method, "?", ptemp.method, sep="_"), "rds")
			} else {
				comp.board.fn <- extensionJoin(paste("mtrain", "comparison_board", load.method, "?", ptemp.method, curr.temp.method.option, paste0("formula", j), intervals[[p]], sep="_"), "rds")
			}
