...
spec <- matrix(
	c('predTemp', 'pt', 1, 'logical',# 'predict temperature at all',
	...
	'predMethod', 'pm', 1, 'character',# 'the prediction method to be used (LM, GAM, NN, RF)',
...
if (is.null(opt$predTemp) || is.null(opt$htype) || is.null(opt$units) || is.null(opt$predMethod)) {
...
temp.res.subfolder.path <- paste(subfolder.path, "temp-results", sep="/")
if (!dir.exists(temp.res.subfolder.path)) dir.create(temp.res.subfolder.path, showWarnings = TRUE, recursive = FALSE, mode = "0777")
...
load.res.subfolder.path <- paste(subfolder.path, "load-results", sep="/")
if (!dir.exists(load.res.subfolder.path)) dir.create(load.res.subfolder.path, showWarnings = TRUE, recursive = FALSE, mode = "0777")


#** COPY PLOT SCRIPTS **#
file.copy(from='plotLoadResults.R', to=load.res.subfolder.path)
file.copy(from='util/plot_helpers.R', to=load.res.subfolder.path)
file.copy(from='plotTempResults.R', to=temp.res.subfolder.path)
...
writeToFile(load.model.chr, output.file)
...
### CROSS VALIDATION ###
########################
for(i in 1:length(gam.temp.models)) {
  PINBALL <- c()
  POSITIONS <- c()
  #-- PRINT TEMP MODEL TYPE TO FILE
  temp.model.chr <- paste0("Temp Model ", i, ": ", gam.temp.models[[i]])
  ...
  if(!use.temp) {
    ...
    for (h in 1:run.len) {
    	...
    	train.features.fn <- paste0(paste("train-temp-features", "instance", h, pred.type, sep="_"), ".rds")
    	saveRDS(train.features, file=paste(tempf.subfolder.path, train.features.fn, sep="/"), compress=TRUE)
    	test.features.fn <- paste0(paste("test-temp-features", "instance", h, pred.type, sep="_"), ".rds")
        ...
        if(gam.temp.models[[i]] == "mean") {
            test.fit <- temp.features[index1:index2, "LAGM"]      
            target <- temp.features[index1:index2, "LAGM"]      
        }
        else {
            temp.model <- trainTempModel(train.features, gam.temp.models[[i]], temp.load.train.dt)
		...
		temp.res.row <- cbind(fit[, c("HASH")], TARGET=target)
		if(h==1) temp.res <- temp.res.row else temp.res <- rbind(temp.res, temp.res.row)

		attr(temp.res.row, 'htype') <- htypeToString(htype)
		attr(temp.res.row, 'horizon') <- test.horizon 
    	temp.res.row.fn <- paste0(paste("temp-model", i, pred.type, "temp-fit-target", "instance", h, sep="_"), ".rds")
    	saveRDS(temp.res.row, file=paste(temp.res.subfolder.path, temp.res.row.fn, sep="/"), compress=TRUE)
  ...
  attr(temp.res, 'horizon') <- run.len 
  temp.res.fn <- paste0(paste("temp-model", i, "all", pred.type, "temp-fit-target", "instance", h, sep="_"), ".rds")
  saveRDS(temp.res, file=paste(temp.res.subfolder.path, temp.res.fn, sep="/"), compress=TRUE)
  ...
  for (j in 1:test.month.len) {
	...
	for (h in 1:length(loop.vars)) { 
		...
		load.model.fn <- paste(model.path, paste0(paste("temp-model", i, "instance", j, pred.type, sep="_"), ".txt"), sep="/")
		...
		if(h==1) {
			...
			load.res.row.fn <- paste0(paste("temp-model", i, pred.type, "load-fit-target", "instance", j, sep="_"), ".rds")
			saveRDS(load.res.row, file=paste(load.res.subfolder.path, load.res.row.fn, sep="/"), compress=TRUE)
		}
  ...
  load.res.fn <- paste0(paste("temp-model",  test.month.len, "all", pred.type, "fit-target", "instance", h, sep="_"), ".rds")
  saveRDS(load.res, file=paste(load.res.subfolder.path, load.res.fn, sep="/"), compress=TRUE)
  ...
  comparison.board.fn <- paste0("comparison_board_tempm_", i, ".rds")
