...
temp.method.path <- paste(temp.path, "gam", sep="/")
if(temp.gamma) {
	temp.method.path <- paste(temp.path, "gamma", sep="/")
}

temp.files <- dir(temp.method.path, pattern = '^predtrain(.*)temp-fit-target', full.names = TRUE)
pattern <- paste0("model", temp.model)
print(temp.files)
print(temp.files[grepl(pattern, temp.files)])
temp.model.files <- temp.files[grepl(pattern, temp.files)]
